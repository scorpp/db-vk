...
#define VK_AUTH_URL "http://oauth.vkontakte.ru/authorize?client_id=" VK_AUTH_APP_ID "&scope=audio,friends&redirect_uri=" VK_AUTH_REDIR_URL "&response_type=token"
...
show_message (GtkMessageType messageType, const gchar *message) {
	...
	dlg =  gtk_message_dialog_new (NULL, //GTK_WINDOW (gtkui_plugin->get_mainwin() ),
...
http_thread_func (void *ctx) {
	...
	char *method_url = g_strdup_printf ("%saudio.search?access_token=%s&q=%s", VK_API_URL, vk_auth_data->access_token, query->query);
	//sprintf(method_url, "%s%s%s", VK_API_URL, "audio.search?access_token=", vk_auth_data->access_token);
